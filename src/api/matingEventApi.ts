...
export const updateMatingEvent = async (id: number, payload: Partial<MatingEvent>) => {
...
export const getAllGroupedByPregnancyResult = async (): Promise<MatingEventsGroup<MatingEvent>[]> => {
  const response = await client.get<MatingEventsGroup<MatingEvent>[]>('/api/matingevents/grouped/pregnancy-result');
  return response.data;
};