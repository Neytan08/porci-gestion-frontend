...
export default function EditBoar() {
	...
	const loadBoarInfo = useCallback(async () => {
		try {
			...
			Alert.alert(
				"Error",
				"No se pudo cargar la informaciÃ³n del verraco. Intente nuevamente.",
			);
		} finally {
			setLoading(false);
		}
	...
	const handleUpdateBoar = async () => {
		...
		try {
			...
			Alert.alert(
				"Ã‰xito",
				"La informaciÃ³n del verraco se ha actualizado correctamente.",
			);
			navigation.goBack();
		} catch (error: any) {
			Alert.alert(
				"Error",
				"No se pudo actualizar la informaciÃ³n del verraco. Intente nuevamente.",
			);
		} finally {
			setLoading(false);
		}
								...
								<TouchableOpacity
									...
									onPress={() => {
										...
										if (!trimmed) {
											Alert.alert(
												"Error",
												"El identificador no puede estar vacÃ­o.",
											);
											return;
										}
