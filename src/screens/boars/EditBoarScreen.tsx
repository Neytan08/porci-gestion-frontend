...
import { getBoarById, updateBoar, Boar } from "../../api/boarsApi";
...
export default function EditBoar() {
    ...
    const [modalEditTagNumberVisible, setModalEditTagNumberVisible] = useState(false);
    ...
    const loadBoarInfo = async () => {
        try {
            ...
            Alert.alert("Error", "No se pudo cargar la información del verraco. Intente nuevamente.");
        } finally {
            setLoading(false);
        }
    }
    ...
    const loadBreedOptions = async () => {
        try {
            ...
            Alert.alert("Error", "No se pudieron cargar las opciones de raza. Intente nuevamente.");
        }
    }
    ...
    const handleUpdateBoar = async () => {
        ...
        try {
            ...
            Alert.alert("Error", "No se pudo actualizar la información del verraco. Intente nuevamente.");
        } finally {
            setLoading(false);
        }
    }
    ...
    return(
        <ScrollView contentContainerStyle={styles.scrollContainer}>
            ...
            <Modal visible={modalEditTagNumberVisible} transparent animationType="slide">
                <View style={[styles.center, { backgroundColor: "rgba(0,0,0,0.4)" }]}>
                    <View style={styles.editTagNumberContainer}>
                        ...
                        <TextInput
                            ...
                            onChangeText={(text) => setBoarInfo({ ...boar, boar_tag_number: text })}
                        />
            ...
            <View>
                ...
                <BreedDropdown 
                    ...
                    onChange={(newBreedId: number) => {
                        const selectedBreed = breedOptions.find((breed) => breed.value === newBreedId);
                        setBoarInfo({
                            ...boar,
                            breed_id: newBreedId,
            ...
            <View style={styles.detailsContainer}>
                {([
                    { label: "Peso(kg)", value: boar?.weight, key: "weight", keyboardType: "numeric" as const },
                    { label: "Largo(cm)", value: boar?.length, key: "length", keyboardType: "numeric" as const },
                    { label: "Descripción", value: boar?.description, key: "description", multiline: true },
                ]).map((item, index) => (
