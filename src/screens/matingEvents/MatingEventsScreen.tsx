...
import { View, FlatList, TouchableOpacity, Pressable, ActivityIndicator, StyleSheet, RefreshControl, Modal, Alert, Image, Text } from "react-native";
...
export default function MattingEventsScreen() {
  ...
  const handleSelectedMatingAction = useCallback((action: "edit" | "delete" | "moreDetails", event: MatingEvent) => {
    ...
    <View style={styles.mainContainer}>
            ...
            return (
              ...
              >
                ...
                <View style={[styles.contentRow, isActive && styles.contentRowActive]}>
                  ...
                  />
                  <Text style={[styles.textCell, { flex: 0.9 }]}>Cerda: {item.breedingsows?.sow_tag_number ?? item.sow_id}</Text>
                  <Text style={[styles.textCell, { flex: 0.8 }]}>
                    Fecha: {item.insemination_date ? item.insemination_date.split("T")[0] : "-"}
                  </Text>
                  <Text style={[styles.textCell, { width: '101%', marginBlock: 8 }]}>
                    Notas: {item.notes ?? "-"}
                  </Text>          
      ...
      >
        <View style={styles.modalActionsContainer}>
          ...
          <Pressable style={styles.modalActionsOverlay} onPress={() => { setActionModalVisible(false); setActionForEvent(null); }} />
          ...
          <View style={styles.modalActionsView}>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
              <EditAction onPress={() => { setActionModalVisible(false); /* luego navega/edit */ handleSelectedMatingAction('edit', actionForEvent!); }} />
              <DetailsAction onPress={() => { setActionModalVisible(false); handleSelectedMatingAction('moreDetails', actionForEvent!); }} />
              <DeleteAction onPress={() => { setActionModalVisible(false); handleSelectedMatingAction('delete', actionForEvent!); }} />
            </View>
          </View>
        </View>
      </Modal>
      ...
      {selectedIds.size > 0 ? (
        ...
        <Image
          ...
          resizeMode="contain"
        />
        <Text style={styles.pinnedSowButtonText}>{`(${selectedIds.size})   `}</Text>
        </TouchableOpacity>
      ):(
        // Floating Add Button 
            ...
            <View style={styles.filterBtnRow}>
              ...
              >
                <Image
                  ...
                  resizeMode="contain"
                />
                <Text style={{ fontWeight: "700",textAlign: "center" }}>Cambiar estado de inseminación</Text>
              </Pressable>
              {/* PDF Extraction */}
              <Pressable
                style={styles.filterBtn}
